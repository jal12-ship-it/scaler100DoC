    ...
    def getPossibleLetters(self, guessed):
        ...
        if len(guessed) > 0:
            for letter in LETTERS:
                check = any(guess in letter for guess in guessed)
                if check is False:
                    possible_letters.append(letter)
         
        else:
            possible_letters = [letter for letter in LETTERS]
        if self.prizeMoney < VOWEL_COST:
            for vowel in VOWELS:
                check = any(pres in vowel for pres in possible_letters)
                if check :
    ...
    def getMove(self, category, obscuredPhrase, guessed):
        ...
        if self.smartCoinFlip():
            for letter in self.SORTED_FREQUENCIES:
                for check in self.getPossibleLetters(guessed):
                    if letter == check:
                        return letter
        else:
